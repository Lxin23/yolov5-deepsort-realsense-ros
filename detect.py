...
def run(
    ...
    if webcam:
        ...
        dataset = LoadStreams(source, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)    # 从摄像头读取数据
        bs = len(dataset)
    elif screenshot:
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        global yolo_output
        yolo_output = str(LOGGER.info(f"{s}{'' if len(det) else '(no detections), '}{dt[1].dt * 1E3:.1f}ms"))
